...
def get_multiple_QPS(data_choice:str='glove',dim:int=25,n_piece:list=[5],nlist: list=[128,256,512,1024],nprobe:list=[1,2,4,6,8,10,15,20,50,100],k:int=10,number:int=1000):
    npiece=n_piece
    nlist = nlist
    nprobe=nprobe
    result_dict={}
...
def plot_multiple_lines_qps( data_dict: dict, data_choice: str = 'glove', dim: int = 25,faiss_style:str='IVFFLat'):
    # 创建文件夹并确定文件路径
    data_name= data_info[data_choice][dim] 
...
if __name__ == "__main__":
    '''
    这里分三步进行：创建多组实验的数据集：
    1. 通过npiece,nlist来进行控制，每个参数类型都是一个列表
    2. 创建完数据集后，通过get_multiple_QPS函数来控制nprobe得到每组实验的数据
    3. 最后通过plot_multiple_lines_qps函数来进行可视化
    
    命名规范：
    总文件夹：index_IVFFLat_QPS
    每个数据的文件夹：glove-25-angular_n5_nlist128
    每个数据的文件：glove-25-angular_nlist128_n5_1.index
    存储图片的文件夹：figure/glove-25-angular_IVFFLAT_QPS
    存储图片的文件：IVFFLat_QPS_multiple_lines.png
    ...
    '''
    # 创建多组实验所采用的数据集
    # get_multiple_data(data_choice='glove',dim=25,n_piece=[5],nlist=[128,256,512,1024])

    # 进行多组实验：输出一个双层字典，key是data_key，value是对应的recall和qps字典
    result_dict=get_multiple_QPS(data_choice='glove',dim=25,n_piece=[5],nlist=[128,256,512,1024])
