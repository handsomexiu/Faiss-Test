# 专门用来测试IVFFLat的qps的
# 这里关键是找到对应的参数
# ivfPQ的创建参数：nlist，M，nbits；搜索参数nprobe
...
# 数据分片
def data_piece(data_choice:str='glove',n_piece:int=5,dim:int=25,nlist:int=100,M:int=5,nbits:int=8)->dict:# 如果data_choice是glove则dim必须是25,50,100,200，如果是sift则dim必须是128，否则会报错
    ...
    for i in range(n_piece):
        data_key=data_name+'_nlist'+str(nlist)+'_M'+str(M)+'_nbits'+str(nbits)+'_n'+str(n_piece)+'_'+str(i+1)
        if i+1 == n_piece:
...
# 创建存储数据的文件
def create_index_folder_choice(data_name:str,n_piece:int=5,index_style:str='IVFPQ',nlist:int=100,M:int=5,nbits:int=8)->str:
    folder_path = f"index_{index_style}_QPS/{data_name}_n{n_piece}_nlist{nlist}_M{M}_nbits{nbits}"
    # 检查文件夹是否存在
...
def create_index_IVFPQ_QPS(data_choice:str='glove',n_piece:int=5,dim:int=25,nlist:int=100,M:int=5,nbits:int=8):
    data_dict,data_name=data_piece(data_choice,n_piece,dim,nlist,M,nbits)
    folder_path = create_index_folder_choice(data_name,n_piece,'IVFPQ',nlist,M,nbits)
    for i in range(n_piece):
        data_key=data_name+'_nlist'+str(nlist)+'_M'+str(M)+'_nbits'+str(nbits)+'_n'+str(n_piece)+'_'+str(i+1)
...
# 获取搜索结果
def get_search_result_IVFPQ_QPS(data_choice:str='glove',n_piece:int=5,dim:int=25,nlist:int=100,M:int=5,nbits:int=8,nprobe:int=1,k:int=10,number:int=100):# number是指定测试集的数量,要和get_test_data函数中的number一致
...
# 计算QPS和召回率
def get_QPS(data_choice:str='glove',n_piece:int=5,dim:int=25,nlist:int=100,M:int=5,nbits:int=8,nprobe:int=1,k:int=10,number:int=100):
    # glove_test,distance_true和neighbors_true暂时没用到
    _,neighbors_true,_=get_test_data_QPS(data_choice,dim,number)# 获取真实id
    start_time = time.time()
    test_id,_=get_search_result_IVFPQ_QPS(data_choice,n_piece,dim,nlist,M,nbits,nprobe,k,number)
    end_time = time.time()
    ...
    for i in npiece:
        for j in nlist:
            for m in M:
                for n in nbits:
                    print(i,j,m,n)
                    create_index_IVFPQ_QPS(data_choice,n_piece=i,dim=dim,nlist=j,M=m,nbits=n)
...
def get_multiple_QPS(data_choice:str='glove',dim:int=25,n_piece:list=[5],nlist: list=[128,512],M:list=[5],
                     nbits:list=[8,12,16],nprobe:list=[1,2,4,6,8,10,15,20,50,100],k:int=10,number:int=1000):
    npiece=n_piece
    nlist = nlist
    M = M
    nbits = nbits
    ...
    for i in npiece:
        for j in nlist:
            for m in M:
                for n in nbits:
                    data_name= data_info[data_choice][dim] 
                    data_key=data_name+'_nlist'+str(j)+'_M'+str(m)+'_nbits'+str(n)+'_n'+str(i)
                    result_dict[data_key]={}
                    qps=[]
                    recall=[]
                    for l in nprobe:
                        mean_reacall,qps_=get_QPS(data_choice,n_piece=i,dim=dim,nlist=j,M=m,nbits=n,nprobe=l,k=k,number=number)
                        qps.append(qps_)
...
# 可视化-------------------------------------------
# 创建存储图片所需要的文件夹
def create_photo_store_qps(data_choice:str='glove',dim:int=25,faiss_style:str='IVFPQ')->str:
    data_name= data_info[data_choice][dim]
...
def plot_multiple_lines_qps( data_dict: dict, data_choice: str = 'glove', dim: int = 25,faiss_style:str='IVFPQ'):
    # 创建文件夹并确定文件路径
    data_name= data_info[data_choice][dim] 
...
if __name__ == "__main__":
    '''
    这里分三步进行：创建多组实验的数据集：
    1. 通过npiece,nlist,M,nbits来进行控制，每个参数类型都是一个列表
    2. 创建完数据集后，通过get_multiple_QPS函数来控制nprobe得到每组实验的数据
    3. 最后通过plot_multiple_lines_qps函数来进行可视化
    
    命名规范：
    总文件夹：index_IVFPQ_QPS
    每个数据的文件夹：glove-25-angular_n5_nlist128_M5_nbits8
    每个数据的文件：glove-25-angular_nlist128_M5_nbits8_n5_1.index
    存储图片的文件夹：figure/glove-25-angular_IVFPQ_QPS
    存储图片的文件：IVFPQ_QPS_multiple_lines.png

    如果命名不规范可能会导致查询不到对应的数据而报错
    '''
    # 创建多组实验所采用的数据集
    # get_multiple_data(data_choice='glove',dim=25,n_piece=[5],nlist=[128],M=[5],nbits=[8,12,16])
    # 这里有一个很奇怪的现象就是nbits=12的时候创建数据库的时间会大幅度的增加，nbits=8的时候只需要10几秒，但是nbits=12的时候用了差不多10分钟
    get_multiple_data(data_choice='glove',dim=25,n_piece=[5],nlist=[128,256,512],M=[5],nbits=[8])

    # 进行多组实验：输出一个双层字典，key是data_key，value是对应的recall和qps字典
    result_dict=get_multiple_QPS(data_choice='glove',dim=25,n_piece=[5],nlist=[128,256,512],M=[5],nbits=[8])
    # ,nprobe=[1,2,4,6,8,10,15,20,50,100],k=10,number=100可以省略，默认就可以
    # 可视化
    plot_multiple_lines_qps(result_dict, data_choice='glove', dim=25,faiss_style='IVFPQ')